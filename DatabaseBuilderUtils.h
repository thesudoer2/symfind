...
inline uint64_t trigram_capacity_for_count(uint64_t count, double max_load_factor = 0.7) {
...
inline constexpr uint64_t trigram_fibonacci_hash(uint32_t trigram_code) {
...
{                                                                                     // lifetime across threads
    ...
    if (!zstd_compressor.compress(file_index_block.data(), file_index_block.size(),
                                  result.file_index_compressed, no_dict, err_msg))
    {
        return {};
    }
    result.file_index_uncompressed_length = file_index_block.size();

    if (!zstd_compressor.compress(file_names_blob.data(), file_names_blob.size(),
                                  result.file_names_compressed, comp_opts, err_msg))
    {
        return {};
    }
...
std::optional<Database::FileTableHeader> commit_file_table(Database &db,
                                                            ...
                                                            std::string *err_msg) noexcept
{
    ...
    if (!Database::write_database_buffer(db, built.file_index_compressed.data(),
                                         built.file_index_compressed.size(), write_offset, err_msg))
    {
        return {};
    }
    ...
    if (!Database::write_database_buffer(db, built.file_names_compressed.data(),
                                         built.file_names_compressed.size(), write_offset, err_msg))
    {
        return {};
    }
                                                      ...
                                                      std::string *err_msg) noexcept
{
    ...
    if (!zstd_compressor.compress(path_index_block.data(), path_index_block.size(),
                                  result.path_index_compressed, no_dict, err_msg))
    {
        return {};
    }
    result.path_index_uncompressed_length = path_index_block.size();

    if (!zstd_compressor.compress(path_names_blob.data(), path_names_blob.size(),
                                  result.path_names_compressed, comp_opts, err_msg))
    {
        return {};
    }
...
std::optional<Database::PathTableHeader> commit_path_table(Database &db,
                                                            FileWrapper::Offset_t &write_offset,
                                                            ...
                                                            std::string *err_msg) noexcept
{
    ...
    if (!Database::write_database_buffer(db, built.path_index_compressed.data(),
                                         built.path_index_compressed.size(), write_offset, err_msg))
    {
        return {};
    }
    ...
    if (!Database::write_database_buffer(db, built.path_names_compressed.data(),
                                         built.path_names_compressed.size(), write_offset, err_msg))
    {
        return {};
    }
                                                          ...
                                                          std::string *err_msg) noexcept
{
    ...
    if (!zstd_compressor.compress(sym_index_block.data(), sym_index_block.size(),
                                  result.symbol_index_compressed, no_dict, err_msg))
    {
        return {};
    }
    result.symbol_index_uncompressed_length = sym_index_block.size();

    if (!zstd_compressor.compress(sym_names_blob.data(), sym_names_blob.size(),
                                  result.symbol_names_compressed, comp_opts, err_msg))
    {
        return {};
    }
    result.symbol_names_uncompressed_length = sym_names_blob.size();

    if (!zstd_compressor.compress(sym_metadata_blob.data(), sym_metadata_blob.size(),
                                  result.symbol_metadata_compressed, no_dict, err_msg))
    {
        return {};
    }
...
std::optional<Database::SymbolIndexHeader> commit_symbol_index(Database &db,
                                                                FileWrapper::Offset_t &write_offset,
                                                                ...
                                                                std::string *err_msg) noexcept
{
    ...
    if (!Database::write_database_buffer(db, built.symbol_index_compressed.data(),
                                         built.symbol_index_compressed.size(), write_offset, err_msg))
    {
        return {};
    }
    ...
    if (!Database::write_database_buffer(db, built.symbol_names_compressed.data(),
                                         built.symbol_names_compressed.size(), write_offset, err_msg))
    {
        return {};
    }
    ...
    if (!Database::write_database_buffer(db, built.symbol_metadata_compressed.data(),
                                         built.symbol_metadata_compressed.size(), write_offset, err_msg))
    {
        return {};
    }

    si_hdr.total_compressed_length = si_hdr.symbol_index_compressed_length +
                                     si_hdr.symbol_names_compressed_length +
                                     si_hdr.symbol_metadata_compressed_length;
    return {si_hdr};
}

std::optional<TrigramIndexBuildResult> build_trigram_index(const HashMap &symtable,
                                                            std::string *err_msg) noexcept
{
    ...
    std::vector<TrigramSlot> trigram_slots(
        capacity, TrigramSlot{.trigram_code = TRIGRAM_EMPTY_SLOT, .posting_offset = 0, .posting_count = 0});
    ...
    if (!zstd_compressor.compress(trigram_index_block.data(), trigram_index_block.size(),
                                  result.trigram_index_compressed, no_dict, err_msg))
    {
        return {};
    }
    ...
    if (!zstd_compressor.compress(postings.data(), total_postings_length,
                                  result.trigram_postings_compressed, no_dict, err_msg))
    {
        return {};
    }
...
std::optional<Database::TrigramIndexHeader> commit_trigram_index(Database &db,
                                                                  FileWrapper::Offset_t &write_offset,
                                                                  ...
                                                                  std::string *err_msg) noexcept
{
    ...
    if (!Database::write_database_buffer(db, built.trigram_index_compressed.data(),
                                         built.trigram_index_compressed.size(), write_offset, err_msg))
    {
        return {};
    }
    ...
    if (!Database::write_database_buffer(db, built.trigram_postings_compressed.data(),
                                         built.trigram_postings_compressed.size(), write_offset, err_msg))
    {
        return {};
    }
